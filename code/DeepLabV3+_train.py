...
# ----------------------------
# 0) 配置：与 U-net_train.py 风格一致
# ----------------------------
@dataclass
class Config:
    # ✅ 改成你当前 U-net 用的同一个根目录
    ...
    # 训练参数
    batch_size: int = 2
    num_workers: int = 0               # Windows 推荐先 0，稳定后再调大
    epochs: int = 200
    lr: float = 1e-4
    weight_decay: float = 1e-4
    seed: int = 42
    amp: bool = True
    save_best_metric: str = "miou"     # "miou" or "mdice"
    save_period: int = 5

    # 输出目录（与你 U-net 保持 expN 结构）
    base_log_dir: str = r"D:\segformer-1.8\logs-deeplabv3plus"
    out_dir: str = ""                  # 留空则自动 expN

    # ✅ 预训练模块（关键）
    encoder_name: str = "resnet50"     # 常用：resnet50 / resnet101 / efficientnet-b0 / mobilenet_v2 ...
    encoder_weights: str = "imagenet"  # 预训练开关： "imagenet" 或 None
    # 可选：DeepLabV3+ 的 decoder 通道设置（默认即可）
    decoder_channels: int = 256

    # 断点续训
    resume: bool = False
    resume_path: str = ""              # 指向 exp?/last.pth 或 best.pth
...
def main():
    ...
    model = smp.DeepLabV3Plus(
        encoder_name=cfg.encoder_name,
        encoder_weights=cfg.encoder_weights,  # "imagenet" 或 None
        in_channels=3,
