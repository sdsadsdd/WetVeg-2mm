...
@dataclass
class EvalConfig:
    # PSPNet 训练输出 best.pth
    model_path: str = r"D:\segformer-1.8\logs-pspnet\exp2\best.pth"
    ...
    # PSPNet 结构参数（默认从 train_config.json 读取；读不到则用默认）
    encoder_name: str = "resnet50"
    encoder_weights: str = "imagenet"  # 仅用于记录
...
def main():
    parser = argparse.ArgumentParser(description="PSPNet 测试集评估脚本（各类别 IoU/Dice/P/R/F1 + 混淆矩阵）")
    parser.add_argument("--model_path", type=str, default=None, help="PSPNet best.pth 路径")
    ...
    parser.add_argument("--disable_postprocessing", action="store_true", help="关闭后处理")
    ...
    # PSPNet 模型壳子（结构需与训练一致）
    model = smp.PSPNet(
        encoder_name=cfg.encoder_name,
        encoder_weights=None,  # 评估只加载 best.pth
        in_channels=3,
    ...
    out_dir = Path(cfg.output_root) / f"PSPNet_{exp_name}_{run_id}"
    ...
    with open(out_dir / "pspnet_test_evaluation_report.json", "w", encoding="utf-8") as f:
    ...
    with open(out_dir / "PSPNET_TEST_EVALUATION_REPORT.md", "w", encoding="utf-8") as f:
        f.write("\n".join(md_lines))
